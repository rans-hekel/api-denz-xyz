...
const listkey = ["Animo", "ANIMO", "animo"];
...
router.get('/info/wikipedia', async (req, res, next) => {
	...
	if(listkey.includes(Apikey)){
       ...
       fetch(encodeURI(`https://api.zeks.xyz/api/wiki?apikey=animobot213&q=${search}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/kuis/caklontong', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
       fetch(encodeURI(`https://api.zeks.xyz/api/cak-lontong?apikey=animobot213`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/kuis/tebaklirik', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
       fetch(encodeURI(`http://94.130.142.91:25621/tebaklirik`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get("/maker/nulis", async (req, res, next) => {
  ...
  if(listkey.includes(apikey)) {
    let hasil = 'https://api.zeks.xyz/api/nulis?text='+ text +'&apikey=animobot213' 
    data = await fetch(hasil).then(v => v.buffer())
